...
const PLAN_ID = process.env.NEXT_PUBLIC_DEMO_PLAN_HOSTED ?? "";
...
export default function HostedSubscribePage() {
  ...
  useEffect(() => {
    if (!PLAN_ID) {
      setError("Demo catalog not configured — set NEXT_PUBLIC_DEMO_PLAN_HOSTED (run scripts/seed-test-merchant.mjs).");
      setState("error");
      return;
    }
    // The merchant already provisioned this plan; hand off to hosted subscribe.
    window.location.assign(`${API_BASE}/subscribe/${PLAN_ID}`);
  }, []);
    ...
    <HostedShell>
      <div style={{ minHeight: 360, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 14, padding: 40, textAlign: "center" }}>
        ...
        <p style={{ color: "#57534e", fontSize: 14, margin: 0 }}>
          {state === "redirecting" && "Redirecting to StellarPay hosted subscription..."}
